...
namespace Crystal
{
    ...
    public class SafeArea : MonoBehaviour
    {
        ...
        private float rightUnits, leftUnits;
        ...
        void ApplySafeArea (Rect r)
        {
            ...
            {             
                ...
                {
                    //if ((LastOrientation == ScreenOrientation.Portrait || LastOrientation == ScreenOrientation.PortraitUpsideDown) && anchorMax.y < 0.96f)
                    //{
                        //anchorMax.y += 0.01f;
                    //}
                ...
                float minSizeSafeArea = 30f;


                var topPixel = Screen.height - (r.position.y + r.size.y);
                var bottomPixel = r.position.y * 0.5f;
                var topRatio = topPixel / Screen.height;
                var bottomRatio = bottomPixel / Screen.height;
                var referenceResolution = canvasScaler.referenceResolution;

                if (ConformBottom)
                {
                    bottomUnits = (referenceResolution.y * bottomRatio) * QualitySettings.resolutionScalingFixedDPIFactor;
                    if (bottomUnits > 0 && bottomUnits < minSizeSafeArea)
                    {
                        bottomUnits = minSizeSafeArea;
                    }
                }
                else
                {
                ...
                if (ConformTop)
                {
                    topUnits = (referenceResolution.y * topRatio) * QualitySettings.resolutionScalingFixedDPIFactor;
                    if (topUnits > 0 && topUnits < minSizeSafeArea)
                    {
                        topUnits = minSizeSafeArea;
                    }
                }
                else
                {
                ...
                var rightPixel = Screen.width - (r.position.x + r.size.x);
                var leftPixel = r.position.x * 0.5f;
                var leftRatio = leftPixel / Screen.width;
                var rightRatio = rightPixel / Screen.width;

                if (ConformLeft)
                {
                    leftUnits = (referenceResolution.x * leftRatio) * QualitySettings.resolutionScalingFixedDPIFactor;
                    if (leftUnits > 0 && leftUnits < minSizeSafeArea)
                    {
                        leftUnits = minSizeSafeArea;
                    }
                }
                else
                {
                ...
                Panel.offsetMin = new Vector2(leftUnits, Panel.offsetMin.y);

                if (ConformRight)
                {
                    rightUnits = (referenceResolution.x * rightRatio) * QualitySettings.resolutionScalingFixedDPIFactor;
                    if (rightUnits > 0 && rightUnits < minSizeSafeArea)
                    {
                        rightUnits = minSizeSafeArea;
                    }
                }
                else
                {
                ...
                Panel.offsetMax = new Vector2(-rightUnits, Panel.offsetMax.y);
                //Debug.Log($"leftUnits {leftUnits} rightUnits {rightUnits} topUnits {topUnits} bottomUnits {bottomUnits} - {LastOrientation.ToString()}");

                //Toast.Show(r.size.y + " - " + Screen.height + " - " + r.position.y + " - " + anchorMax.y + " - " + topUnits + " - " + bottomPixels, 10000000);
            }
