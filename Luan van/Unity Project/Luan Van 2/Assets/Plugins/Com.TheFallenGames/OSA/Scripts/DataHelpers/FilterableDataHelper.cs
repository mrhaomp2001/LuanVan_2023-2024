	...
	public class FilterableDataHelper<T> : IEnumerable<T>
	{
		...
		public Predicate<T> FilteringCriteria
        {
			set
            {
				...
				_Adapter.ResetItems(_CurrentList.Count);
			}
        }
        ...
        public int Count { get { return _CurrentList.Count; } }
		...
		IEnumerator<T> IEnumerable<T>.GetEnumerator() { return _CurrentList.GetEnumerator(); }
		
		IEnumerator IEnumerable.GetEnumerator() { return _CurrentList.GetEnumerator(); }
		#endregion
		...
		public void InsertItems(int index, IList<T> models, bool freezeEndEdge = false)
		{
			...
			if (_Adapter.InsertAtIndexSupported)
				...
				_Adapter.ResetItems(_CurrentList.Count, freezeEndEdge, _KeepVelocityOnCountChange);
		}
		...
		public void InsertOne(int unfilteredIndex, T model, bool freezeEndEdge = false)
		{
			...
			if (isFilteredModel)
            {
				...
				_FilteredItemIndex.Insert(filteredIndex, filteredIndex);
			}
